...
- 商品名称以单据表格中的印刷文字为准，逐字逐词全部读完，绝对不能省略任何部分
- 如果图片模糊、字迹不清晰导致某个字段无法确认，必须明确告诉用户"商品名称看不清楚，能否补拍一张商品名称处的特写？"，不要猜测填入
- 单据上"销货单位/供货单位"的公司名 → 供应商（优先用印刷体公司名，不要用底部手写人名）
- 手写人名逐字辨认，正式姓名（如"王丽敏"）不要误读为口头称谓（如"王阿姨"）
- 识别完毕后，先列出识别到的所有字段让用户确认，再执行录入
