<template>
  <div class="page-container">
    <el-card>
      <ScTable ref="tableRef" :api-obj="getOtherInList" :params="searchForm">
        <template #search>
          <el-input v-model="searchForm.in_no" placeholder="入库单编号" clearable style="width: 180px" />
        </template>
        <template #toolbar>
          <el-button type="primary" :icon="Plus" @click="openForm()">新增</el-button>
        </template>

        <el-table-column prop="in_no" label="入库单编号" width="160" />
        <el-table-column prop="goods_name" label="商品名称" min-width="150" />
        <el-table-column prop="num" label="数量" width="100" />
        <el-table-column prop="warehouse_name" label="仓库名称" width="150" />
        <el-table-column prop="reason" label="入库原因" min-width="160" show-overflow-tooltip />
    ...
    <ScForm ref="formRef" title="新增其他入库" @submit="handleSubmit">
      <template #default="{ form }">
        ...
        <el-form-item label="入库原因" prop="reason">
          <el-input v-model="form.reason" type="textarea" placeholder="请输入入库原因" />
        </el-form-item>
      </template>
    </ScForm>
  </div>
...
import { getOtherInList, createOtherIn, deleteOtherIn } from '@/api/warehouse'
