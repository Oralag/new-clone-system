      ...
      <el-card>
        <ScTable ref="tableRef" :api-obj="getProcurePlanList"
          ...
          <el-table-column label="计划金额" width="120" align="right">
            <template #default="{ row }">
              <span style="color:#165dff;font-weight:500">¥{{ parseItems(row.goods_info).reduce((s: number, i: any) => s + (i.num||0)*(i.price||0), 0).toFixed(2) }}</span>
            </template>
          </el-table-column>
    ...
    <el-dialog v-model="inhouseVisible" title="采购计划入库" width="520px" append-to-body>
      <el-form :model="inhouseForm" label-width="90px" v-if="inhouseRow">
        <el-form-item label="采购计划">
          <span style="color:#4e5969">{{ inhouseRow.order_sn }}</span>
        </el-form-item>
        <el-form-item label="供应商">
          <span style="color:#4e5969">{{ inhouseRow.supplier_name || '—' }}</span>
        </el-form-item>
