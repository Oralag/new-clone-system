<template>
  <div class="page-container">
    <el-card>
      <ScTable ref="tableRef" :api-obj="getOutsourceInhouseList"
          del-path="/outsource/inhouse/batchDel"
          export-file-name="外协入库" :params="searchForm">
        <template #search>
          <el-form inline>
            <el-form-item label="入库编号">
              <el-input v-model="searchForm.in_no" clearable style="width:180px" />
            </el-form-item>
        ...
        <el-table-column prop="in_no" label="入库编号" min-width="140" />
...
async function handleSubmit(data: any) {
  ...
  try {
    await createOutsourceInhouse(data)
...
async function handleDelete(id: number) {
  await ElMessageBox.confirm('确定删除该委外入库记录？', '提示', { type: 'warning' })
  await deleteOutsourceInhouse(id)
