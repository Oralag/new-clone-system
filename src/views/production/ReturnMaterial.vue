<template>
  <div class="return-material-page">
    ...
    <div v-if="!showForm">
      <el-card>
        <ScTable ref="tableRef" :api-obj="getReturnMaterialList"
          del-path="/production/returnMaterial/batchDel"
          export-file-name="生产退料" :params="searchForm">
          <template #search>
            <el-input v-model="searchForm.return_no" placeholder="退料编号" clearable style="width:160px" />
            <el-input v-model="searchForm.goods_name" placeholder="商品名称" clearable style="width:160px" />
            <el-button type="primary" @click="tableRef?.loadData()">查询</el-button>
            <el-button @click="resetSearch">重置</el-button>
          </template>
          <template #toolbar>
            <el-button type="primary" :icon="Plus" @click="openAdd">新增退料</el-button>
          </template>
          <el-table-column prop="return_no" label="退料编号" min-width="150" />
          <el-table-column prop="plan_name" label="关联生产计划" min-width="150" />
          <el-table-column prop="return_date" label="退料日期" width="110">
            <template #default="{ row }">{{ (row.return_date||row.created_at||'').slice(0,10) }}</template>
          </el-table-column>
          <el-table-column prop="warehouse_name" label="退回仓库" min-width="110" />
          <el-table-column label="退料总价" width="110" align="right">
    ...
    <div v-else class="form-page">
      <div class="form-topbar">
        <div class="form-topbar-left">
          ...
          <span class="form-title">{{ isView?'查看退料单':fd.id?'编辑退料单':'新增退料单' }}</span>
      ...
      <div class="form-body">
        <div class="form-section">
          <el-row :gutter="16">
            <el-col :span="6">
              <div class="field-row">
                <span class="field-label">退料编号</span>
                <el-input v-model="fd.return_no" placeholder="不填自动生成" style="flex:1" :disabled="isView" />
              </div>
            </el-col>
            <el-col :span="6">
              <div class="field-row">
                <span class="field-label required">退料日期</span>
                <el-date-picker v-model="fd.return_date" type="date" value-format="YYYY-MM-DD" style="flex:1" :disabled="isView" />
              </div>
            </el-col>
            <el-col :span="6">
              <div class="field-row">
            ...
            <el-col :span="6">
              <div class="field-row">
                ...
                <el-input v-model="fd.returner" placeholder="退料人" style="flex:1" :disabled="isView" />
              </div>
            </el-col>
        ...
        <div class="goods-toolbar" v-if="!isView">
          ...
          <span class="goods-summary">退料总价：<b>{{ totalPrice.toFixed(2) }}</b></span>
        </div>
        <div class="goods-summary-view" v-else>退料总价：<b>{{ totalPrice.toFixed(2) }}</b></div>

        <el-table :data="fd.items" border size="small" style="width:100%" empty-text="请添加退料商品">
          ...
          <el-table-column label="退料数量" width="110">
            <template #header>退料数量<el-button v-if="!isView" link type="primary" size="small" @click="batchSet('num','退料数量')">批量</el-button></template>
            <template #default="{ row }">
          ...
          <el-table-column label="入库单价" width="110">
            <template #header>入库单价<el-button v-if="!isView" link type="primary" size="small" @click="batchSet('in_price','入库单价')">批量</el-button></template>
            <template #default="{ row }">
              <el-input-number v-if="!isView" v-model="row.in_price" :min="0" :precision="4" controls-position="right" size="small" style="width:100%" @change="calcRow(row)" />
          ...
          <el-table-column label="退料原因" min-width="120">
            <template #default="{ row }">
              <el-input v-if="!isView" v-model="row.reason" size="small" placeholder="原因" />
        ...
        <div class="form-footer">
          <div class="footer-summary">合计：数量 <b>{{ totalNum.toFixed(2) }}</b>&nbsp;&nbsp;总价 <b style="color:#16a34a">{{ totalPrice.toFixed(2) }}</b></div>
...
import { getReturnMaterialList, createReturnMaterial, deleteReturnMaterial, auditReturnMaterial } from '@/api/production'
...
async function loadWarehouses(){try{const r=await getWarehouseList({list_rows:200});warehouseOptions.value=r.data?.list||r.data?.rows||r.data?.data||[]}catch{}}
function onWarehouseChange(id:any){const w=warehouseOptions.value.find(x=>x.id===id);fd.warehouse_name=w?.name??'';fd.items.forEach(r=>{if(!r.warehouse_id){r.warehouse_id=id;r.warehouse_name=w?.name??''}})}

function defaultFd(){return{id:0,status:0,return_no:'',return_date:new Date().toISOString().slice(0,10),warehouse_id:null as any,warehouse_name:'',returner:'',plan_id:0,plan_name:'',remark:'',items:[] as any[],total_price:0}}
...
function confirmGoods(){pickerSel.value.forEach(g=>fd.items.push({goods_id:g.id,goods_name:g.name,goods_sn:g.goods_sn||'',spec:g.spec||'',unit_name:g.unit_name||'',num:1,in_price:0,row_total:0,reason:''}));goodsPickerVisible.value=false}
function addEmptyRow(){fd.items.push({goods_id:0,goods_name:'',goods_sn:'',spec:'',unit_name:'',num:1,in_price:0,row_total:0,reason:''})}
...
async function handleSave(){
  if(!fd.return_date){ElMessage.warning('请选择退料日期');return}
  if(!fd.warehouse_id){ElMessage.warning('请选择退回仓库');return}
  if(!fd.items.length){ElMessage.warning('请添加商品');return}
  saving.value=true
  try{await createReturnMaterial({...fd,goods_info:JSON.stringify(fd.items),total_price:totalPrice.value});ElMessage.success('保存成功');backToList()}catch{}finally{saving.value=false}
}
async function doAudit(row:any,status:number){const labels:Record<number,string>={1:'审核',2:'驳回',0:'反审核'};await ElMessageBox.confirm(`确定${labels[status]}该退料单？`,'提示',{type:'warning'});try{await auditReturnMaterial(row.id,status);ElMessage.success('操作成功');tableRef.value?.refresh()}catch{}}
async function handleDelete(id:number){await ElMessageBox.confirm('确定删除？','提示',{type:'warning'});await deleteReturnMaterial(id);ElMessage.success('删除成功');tableRef.value?.refresh()}
