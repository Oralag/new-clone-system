...
const defaultFd = () => ({
  ...
  fund_name: '',
  pay_method: 'bank',
  pay_date: new Date(Date.now() + 8 * 3600000).toISOString().slice(0, 10),
  order_no: '',
  remark: '',
})
...
async function handleSave() {
  ...
  saving.value = true
  try {
    const { contact_type, contact_id, fund_id, fund_name, order_no, pay_method, ...rest } = fd
    const payload: any = {
      ...rest,
      fund_id: fund_id ?? 0,
      fund_name: fund_name || '',
      pay_type: pay_method || 'cash',
